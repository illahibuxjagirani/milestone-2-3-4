...
function addNewAcadamicField(): void {
...
function addNewWorkExpField(): void {
...
function generateCV(): void {
  ...
  // Handle social links
  const fbLink = document.getElementById('fbField') as HTMLInputElement;
  const linkedInLink = document.getElementById('linkedInField') as HTMLInputElement;
  const githubLink = document.getElementById('githubField') as HTMLInputElement;
  ...
  const skillsFields = document.querySelectorAll('.skills-field') as NodeListOf<HTMLTextAreaElement>;
  let skillsList = "";
  ...
  const workFields = document.querySelectorAll('.work-field') as NodeListOf<HTMLTextAreaElement>;
  let workList = "";
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const addAcademicBtn = document.getElementById('addAcademicBtn') as HTMLButtonElement;
  if (addAcademicBtn) {
  ...
  const addWorkExpBtn = document.getElementById('addWorkExpBtn') as HTMLButtonElement;
  if (addWorkExpBtn) {
    addWorkExpBtn.addEventListener('click', addNewWorkExpField);
  }

  // Event listener for "Generate CV" button
  const generateButton = document.getElementById('generateCVBtn') as HTMLButtonElement;
  if (generateButton) {
    generateButton.addEventListener('click', generateCV);
  }

  // Event listener for "Print CV" button
  const printButton = document.getElementById('printCVBtn') as HTMLButtonElement;
  if (printButton) {
